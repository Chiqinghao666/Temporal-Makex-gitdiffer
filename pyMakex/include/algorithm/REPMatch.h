...
inline std::pair<int, int> REPMatchBasePTime_Pivot_Match(
    ...
    std::map<std::pair<DataVertexPtr, DataVertexPtr>, std::pair<int, int>>
        &ml_model,
    bool using_cache = true,
    std::set<int> *pivot_match = nullptr,
    int who_is_pivot_label = 0,
    std::set<int> *pivot_match_ = nullptr,
    int who_is_pivot_label_ = 1,
    std::vector<std::pair<DataVertexPtr, DataVertexPtr>> *positive_result =
        nullptr,
    std::vector<std::pair<DataVertexPtr, DataVertexPtr>> *all_result =
        nullptr,
  ...
  if (!info_set.count("Variable")) {
    ...
    for (auto x_candidate : candidate_set.find(x_ptr)->second) {
      for (auto y_candidate : candidate_set.find(y_ptr)->second) {
        ...
        if (positive_flag) {
          positive_count++;
          pattern_positive_match[x_candidate->id()].push_back(y_candidate->id());
          // only recommendation module need
          if (positive_result != nullptr) {
            positive_result->emplace_back(x_candidate, y_candidate);
    ...
    if(who_is_pivot_label == 1){
    //user
    auto it_x = candidate_set.find(x_ptr);
    if (it_x != candidate_set.end()) {
      auto &candidates = it_x->second;
      for (auto &candidate : candidates) {
        if (candidate->label() == who_is_pivot_label) {
          pivot_match->insert(candidate->id());
        }
      }
    }
  }

  if(who_is_pivot_label == 0){
    //user
    auto it_y = candidate_set.find(y_ptr);
    if (it_y != candidate_set.end()) {
      auto &candidates = it_y->second;
      for (auto &candidate : candidates) {
        if (candidate->label() == who_is_pivot_label) {
          pivot_match->insert(candidate->id());
        }
  ...
  if(who_is_pivot_label_ == 1){
    //user
    auto it_x = candidate_set.find(x_ptr);
    if (it_x != candidate_set.end()) {
      auto &candidates = it_x->second;
      for (auto &candidate : candidates) {
        if (candidate->label() == who_is_pivot_label_) {
          pivot_match_->insert(candidate->id());
        }
      }
    }
  }

  if(who_is_pivot_label_ == 0){
    //user
    auto it_y = candidate_set.find(y_ptr);
    if (it_y != candidate_set.end()) {
      auto &candidates = it_y->second;
   ...
   double conf = (1.0 * positive_count) / (all_count);
    // std::cout << "REP ID: "<< rep_id << " REPMatch.h supp is " << positive_count << " " << all_count << ", conf is: " << conf << std::endl;
    return {positive_count, all_count};
  }
  ...
  if (using_cache) {
    ...
    std::map<PatternVertexPtr, PatternVertexPtr> leaf_map;
    BuildLeafMap<Pattern, DataGraph>(rep, leaf_map);
    for (const auto& pair : leaf_map) {
      std::cout << "leaf_map Key: " << pair.first->id() << " leaf_map Values: " << pair.second->id() << std::endl;
    }
    CacheType cache;
    BuildCache(rep, data_graph, candidate_set, cache, leaf_map);
    std::map<DataVertexPtr, CandidateSet> x_candidate_set_cache,
    ...
    BuildCandidateSetCache<Pattern, DataGraph>(rep, candidate_set, leaf_map,
                                               ...
                                               y_candidate_set_cache);                                 
    auto t_end = clock();
    // std::cout << "Build Cache time is "
    //          << (1.0 * t_end - t_begin) / CLOCKS_PER_SEC << std::endl;
    std::cout << "x_candidate_set_cache.size()" << x_candidate_set_cache.size() << std::endl;
    std::cout << "y_candidate_set_cache.size()" << y_candidate_set_cache.size() << std::endl;
    for(auto &pair : x_candidate_set_cache){
      std::cout << "x_candidate_set_cache Key: " << pair.first->id() << " x_candidate_set_cache Values: "  << std::endl;
      for (const auto& pair_ : pair.second) {
        std::cout << "all result Key: " << pair_.first->id() << " all result Values: " << pair_.second.size() << std::endl;
      }
    }
    for(auto &pair : y_candidate_set_cache){
      std::cout << "y_candidate_set_cache Key: " << pair.first->id() << " y_candidate_set_cache Values: " << std::endl;
      for (const auto& pair_ : pair.second) {
        std::cout << "all result Key: " << pair_.first->id() << " all result Values: " << pair_.second.size() << std::endl;
      }
    }
    ...
    for (const auto &[x_candidate, x_cache] : x_candidate_set_cache) {
      ...
      for (const auto &[y_candidate, y_cache] : y_candidate_set_cache) {
        ...
        CandidateSet refine_candidate_set(x_cache);
        std::cout << "refine_candidate_set end" << std::endl;
        for (const auto &it : y_cache) {
          refine_candidate_set.emplace(it.first, it.second);
        }
        std::cout << "refine_candidate_set: " << refine_candidate_set.size() << std::endl;
        if (!PredicateCheck(rep, refine_candidate_set)) {
          continue;
        }
        if(who_is_pivot_label == 1){    
          pivot_match->insert(x_candidate->id());
          std::cout << "0515 pivot_match->insert(x_candidate->id());" << std::endl;

        }

        if(who_is_pivot_label == 0){
          pivot_match->insert(y_candidate->id());
          std::cout << "pivot_match->insert(y_candidate->id());" << std::endl;
        }

        if(who_is_pivot_label_ == 1){
          pivot_match_->insert(x_candidate->id());
          std::cout << "pivot_match->insert(x_candidate->id());" << std::endl;

        }

        if(who_is_pivot_label_ == 0){
          pivot_match_->insert(y_candidate->id());
          std::cout << "pivot_match->insert(y_candidate->id());" << std::endl;
        }
    ...
    // std::cout << "pivot_match size: " << pivot_match->size() << std::endl;
    // std::cout << "REPMatch.h count is " << positive_count << " " << all_count << std::endl; 
  } else {
    std::vector<PatternVertexPtr> x_leaves, y_leaves;
    GetLeaves(x_ptr, x_leaves);
    ...
    std::map<PatternVertexPtr, PatternVertexPtr> variable_nodes;
    for (auto &single_predicate : rep.x_prediate()) {
      if(single_predicate->info() != "Variable"){
        continue;
      }
      else{
        variable_nodes[single_predicate->GetX().first] = single_predicate->GetY().first;
        variable_nodes[single_predicate->GetY().first] = single_predicate->GetX().first;
      }
    }
    ...
    for (auto x_candidate : candidate_set.find(x_ptr)->second) {
      ...
      for (auto y_candidate : candidate_set.find(y_ptr)->second) {
        ...
        if (!ml_model.empty() &&
          ...
          auto &y_cache = y_candidate_set_cache[y_candidate];
          CandidateSet refine_candidate_set;
            for (auto &variable : variable_nodes) {
              if (x_cache.count(variable.first)) {
                refine_candidate_set[variable.first] = x_cache[variable.first];
              } else {
                refine_candidate_set[variable.first] = y_cache[variable.first];
              }
            }
